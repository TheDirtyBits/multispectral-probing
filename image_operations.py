...
def transform_image(image_name, r_text, g_text, b_text, gray=None, save=False, save_name=None):
    dataset: gdal.Dataset = gdal.Open(image_name, gdal.GA_ReadOnly)
    x_size = dataset.RasterXSize
    y_size = dataset.RasterYSize
    r_array = dataset.GetRasterBand(3).ReadAsArray()
    ...
    if save:
        ...
        new_dataset: gdal.Dataset = driver.CreateCopy(save_name, dataset, strict=0)
        ...
        format_save("preview_"+image_name[:-3]+"png", x_size, y_size, *values_rgb)
